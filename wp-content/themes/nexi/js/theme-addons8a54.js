...
function wglButton() {
    const buttons = document.querySelectorAll('.magnetic-yes .wgl-button, .magnetic-yes .wpcf7-submit');
    ...
    buttons.forEach(button => {
        const wrapper = button.closest('.magnetic-yes');
        if (!wrapper) return;
        ...
        const strength = button.dataset.magneticStrong
            ...
            : parseFloat(wrapper.dataset.magneticStrong) || 0.4;

        const getThreshold = () => {
            const rect = button.getBoundingClientRect();
            const multiplier = button.dataset.magneticThreshold
                ? parseFloat(button.dataset.magneticThreshold)
                : parseFloat(wrapper.dataset.magneticThreshold) || 2;
        ...
        const onMouseMove = (e) => {
            ...
            if (distance < threshold) {
                const force = (distance / threshold) * 50 * strength;
        ...
        wrapper.addEventListener('mouseenter', updateRect, { signal });
...
function wglCursorInit() {
    ...
    if('object' === typeof gsap && cursorPointerWrap.length){
        ...
        window.addEventListener('mousemove', (e) => {
            let cursorDuration = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--nexi-cursor-duration').trim()) || 0.35;
            let duration = parseFloat(jQuery(cursorPointer).css('--duration')) || cursorDuration;

            let cursorFollowerDuration = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--nexi-cursor-follower-duration').trim()) || 0.9;
            let durationFollower = parseFloat(jQuery(cursorFollower).css('--duration')) || cursorFollowerDuration;

            let cursorAnimation = getComputedStyle(document.documentElement).getPropertyValue('--nexi-cursor-link-animation').trim();
            ...
            const cursorStyles = {
                ...
                'link-animation-style-2': {
                    ...
                    follower: { opacity: 0.6, scale: 1.4, backgroundColor: jQuery(cursorFollower).css('--nexi-cursor-follower-color')}
                },
    ...
    if (cursorText.length) {
        cursorText.each(function() {
            ...
            let timerId;
            const blendMode = getComputedStyle(document.documentElement).getPropertyValue('--nexi-cursor-blend').trim();

            cursorEventHtml.on('mouseenter', function () {
                if (appendElement) {
                    cursorPointer.parent().css('--nexi-cursor-blend', 'normal');
                    cursorPointer
                        .empty()
                    ...
                    cursorPointer.css('--nexi-cursor-point-size', cursorSize);
                    cursorPointer.css('--animation',linkAnimation);

                    cursorFollower.css('--nexi-cursor-follower-size',cursorFollowerSize);
                    cursorFollower.css('--nexi-cursor-follower-color', cursorFollowerColorBg);
                ...
                if (appendElement) {
                    ...
                    cursorPointer.parent().css('--nexi-cursor-blend', blendMode ?? 'normal');
                    ...
                    if ( e.relatedTarget ) {
                        ...
                        if (newTarget.length > 0 && newTarget.hasClass('wgl-cursor-text')) {
                            ...
                            cursorPointer.css('color','var(--nexi-cursor-point-color)');
                            jQuery(cursorPointer)[0].style.removeProperty('--duration');
                            jQuery(cursorPointer)[0].style.removeProperty('--animation');
                            jQuery(cursorPointer)[0].style.removeProperty('--nexi-cursor-point-size');
                            jQuery(cursorFollower)[0].style.removeProperty('--nexi-cursor-follower-size');
                            jQuery(cursorFollower)[0].style.removeProperty('--nexi-cursor-follower-color');
...
function wglDynamicStyles() {
    var style = jQuery('#nexi-footer-inline-css');
...
function wglInfoboxInit() {
    const selector = ".animation_content .wgl-striped_content, .animation_content .wgl-infobox-content_wrapper, .animation_button .wgl-button-wrapper";
        ...
        _move: function(el, animate) {
            ...
            if (el.hasClass('wgl-tabs_header') || el.parent().hasClass('wc-tabs')) {
                ...
                cssProperties = {
                    ...
                    marginTop: 'calc(' + (el.children().outerHeight() - 40) / 2 + 'px)',
                };
            }
...
function wglRandomFadeInit(item) {
    let widgetSelector = '.wgl-random_fade .wgl-fade_item',
        $items = item ? jQuery(item).find(widgetSelector) : jQuery(widgetSelector);

    if (!$items.length) return;

    function processItem($el) {
        const hideMobile = $el.data('hide-mobile') === 'yes';
        const bp = parseInt($el.data('hide-breakpoint')) || 767;

        if (hideMobile && window.innerWidth <= bp) {
        ...
        function getSafePosition() {
            const parent = $el.closest('.wgl-random_fade')[0];
            ...
            return {
                left: Math.random() * Math.max(0, parentW - elW),
                top: Math.random() * Math.max(0, parentH - elH),
            };
        }
        ...
        function setPosition() {
            ...
            gsap.set($el, {
                left: pos.left,
                top: pos.top,
                rotate: 30 - 60 * Math.random(),
                zIndex: 11 + Math.floor(Math.random() * 5),
                pointerEvents: 'none',
            });
        }
        ...
        $el.on('mouseenter', fadeOut);
    }

    function initAll() {
        $items.each(function () {
            processItem(jQuery(this));
        });
    }
...
function wglScrollGalleryInit($scope = false) {
    ...
    if ($scope && $scope.length) {
        $scrollGallery = $scope.find('.wgl-scroll-gallery');
    } else {
        $scrollGallery = jQuery('.wgl-scroll-gallery');
    }

    if (!$scrollGallery.length) return;

    if (!window.WGLScrollFlipRegistered) {
        gsap.registerPlugin(Flip, ScrollTrigger);
        window.WGLScrollFlipRegistered = true;
    }

    if (!document.documentElement.dataset.wglAutoScroll) {
        document.documentElement.style.scrollBehavior = 'auto';
        document.documentElement.dataset.wglAutoScroll = 'true';
    }

    let wglFlipContexts = new WeakMap();

    function initScroll($gallery) {

        const gallery = $gallery[0];
        const prevCtx = wglFlipContexts.get(gallery);

        if (prevCtx) {
            prevCtx.revert();
            wglFlipContexts.delete(gallery);
        }

        const $wrapper = $gallery.closest('.wgl-scroll-gallery-wrapper');
        const disableMobile = $wrapper.data('responsive') === true || $wrapper.data('responsive') === 'true';
        const mobileBp = parseInt($wrapper.data('mobile-bp'), 10) || 1200;

        $gallery.parent().removeClass('responsive');

        if (disableMobile && window.innerWidth <= mobileBp && !$scope) {
            $gallery.parent().addClass('responsive');
            $gallery.find('.gallery__item').removeClass('is-active is-active-strong');
            return;
        }
        ...
        const ctx = gsap.context(() => {
            ...
            if ($gallery.hasClass('gallery--row')) options.flip = { absoluteOnLeave: true, scale: false };
            if ($gallery.hasClass('gallery--stack-scale')) options.flip = { scale: false };
            if ($gallery.hasClass('gallery--grid10')) {
                options.stagger = 0.05;
                options.flip = { absolute: true, scale: false };
                options.scrollTrigger = { start: 'center center', end: '+=900%' };
            }
            if ($gallery.hasClass('gallery--bento')) options.flip = { scale: false };

            const settings = jQuery.extend(true, {}, defaults, options);

            const $caption = $gallery.find('.caption');
            const $items = $gallery.find('.gallery__item');
            const $inners = $items.children();

            const triggerEl = $gallery[0];
            const pinEl = $gallery.parent()[0];
            ...
            function setActive($item) {
                ...
                if (activeItem) activeItem.removeClass('is-active is-active-strong');
            ...
            function addStrong() {
                if (activeItem) {
                    activeItem.addClass('is-active-strong');
                }
            }

            function removeStrongOnly() {
                if (activeItem) {
                    activeItem.removeClass('is-active-strong');
                }
            }

            $items.on('mouseenter.wglHover', function () {
                if (!activeItem || activeItem[0] !== this) removeStrongOnly();
                setActive(jQuery(this));
            });

            jQuery(document).on('mousemove.wglGalleryHover', function (e) {
                const el = document.elementFromPoint(e.clientX, e.clientY);
                if (!el) return;

                const $el = jQuery(el);
                const $item = $el.closest('.gallery__item');
                const isCaption = $el.closest('.caption').length;

                if ($item.length) {
                    if (!activeItem || activeItem[0] !== $item[0]) removeStrongOnly();
                    setActive($item);
                    return;
                }
            ...
            $gallery.addClass('gallery--switch');

            const state = Flip.getState($items.add($caption), {
                props: 'filter,opacity'
            });

            $gallery.removeClass('gallery--switch');
            ...
            return () => {
                $items.off('.wglHover');
                jQuery(document).off('mousemove.wglGalleryHover');
                $items.each((i, el) => {
        ...
        wglFlipContexts.set(gallery, ctx);
    }

    $scrollGallery.each(function () {
        initScroll(jQuery(this));
    });
        ...
        $showcase_init.each(function() {
            ...
            function responsive() {
                titles.each(function() {
                    ...
                    let subtitle = parent.children('.showcase__subtitle');
                    let totalWidth = Math.floor(title.outerWidth(true));
                    if (subtitle.length) {
                        totalWidth += Math.floor(subtitle.outerWidth(true));
                    }
                    parent.css({ '--sc-title-width': totalWidth + 'px' });
                });
